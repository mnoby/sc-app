...
const stateChange=stateNum=>{
  console.log(`state >> ${stateNum}`)
  if(stateNum==1){
    expandState.value = !expandState.value
  }else if(stateNum==2){
    expandState2.value = !expandState2.value
  } else{
    expandState3.value = !expandState3.value
  }
        ...
        >
          <VCardItem>
            ...
            <VExpandTransition>
              ...
              >
                <VListItemTitle
                  v-for="(item) in prop.parseOrder.package"
                  :key="item.product_name"
                  :ripple="false"
                  class="text-subtitle-2 text-high-emphasis text-capitalize"
                >
                  ...
                  <VBtn
                    ...
                    size="small"
                    width="100vw"
                    class="font-weight-medium align-center justify-start"
                    color="black"
                    :ripple="false"
                    :append-icon="subExpandState2? 'bx-chevron-up' : 'bx-chevron-down' "
                    @click="subExpandState2 = !subExpandState2"
                  >
                    <VRow>
                      <VCol>
                        {{ item.product_name }}
